% Appendix A

\chapter{Appendix E. Semi-structured Questionnaire for Contextual Enquiry} % Main appendix title
\clearpage
\label{AppendixE} % For referencing this appendix elsewhere, use \ref{AppendixA}

\lhead{Appendix E. \emph{Contextual Enquiry Questionnaire}} % This is for the header on each page - perhaps a shortened title
\includepdf[pages=-,frame,pagecommand={},width=\textwidth,offset=90 -25]{Pdfs/contextualenqsurvey.pdf}
\clearpage
%\includepdf[pages=-,frame,pagecommand={},width=\textwidth,offset=90 0]{Pdfs/midpoint_interm.pdf}

%\newcommand{\insertrep}[1]{%
%\hspace{-2.4cm}
%\fbox{\includegraphics[page=1,scale=0.8]{#1}}
%\includegraphics[page=1,scale=0.8]{#1}
%\includepdf[scale=0.75,pages=1,frame]{#1}
%}

%\subsection{Interesting Letter}
%\insertrep{Pdfs/baseline_ben.pdf}
%\begin{center}
%\includepdf[pagecommand={},scale=0.9]{Pdfs/baseline_ben.pdf} 
%\end{center}
